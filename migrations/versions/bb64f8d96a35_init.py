...
Revision ID: bb64f8d96a35
Revises: 
Create Date: 2021-11-11 00:52:24.575736
...
def upgrade():
    ...
    op.create_index(op.f('ix_organization_access_code'), 'organization', ['access_code'], unique=True)
    ...
    sa.PrimaryKeyConstraint('id', 'major', 'minor')
    )
    ...
    sa.Column('longitude', sa.Float(), nullable=False),
    sa.Column('latitude', sa.Float(), nullable=False),
    sa.Column('beacon_id', sqlmodel.sql.sqltypes.AutoString(length=35), nullable=False),
    sa.Column('beacon_major', sa.Integer(), nullable=False),
    sa.Column('beacon_minor', sa.Integer(), nullable=False),
    sa.Column('path_id', sa.Integer(), nullable=True),
    sa.Column('id', sa.Integer(), nullable=True),
    sa.ForeignKeyConstraint(['beacon_id', 'beacon_major', 'beacon_minor'], ['beacon.id', 'beacon.major', 'beacon.minor'], ),
    sa.ForeignKeyConstraint(['path_id'], ['path.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_point_beacon_id'), 'point', ['beacon_id'], unique=False)
    op.create_index(op.f('ix_point_beacon_major'), 'point', ['beacon_major'], unique=False)
    op.create_index(op.f('ix_point_beacon_minor'), 'point', ['beacon_minor'], unique=False)
...
def downgrade():
    ...
    op.drop_index(op.f('ix_point_beacon_minor'), table_name='point')
    op.drop_index(op.f('ix_point_beacon_major'), table_name='point')
